...
//	private GuidaRepository guidaRepo;
	...
	//prima ricerco il prodotto tramite id, poi passo quel prodotto alla repo immagini per vedere tutte le img ad esso associate
	...
	//passo l'id del prodotto, voglio un immagine per quel prodotto (qui inizio a crearla, ma non è convertita)
	//ritorna infatti come valore una imageForm con un prodotto associato (alla fine posso associarne più di uno)
	public ImageForm createImageForm(Integer prodottoId) {
		Prodotto prodotto = prodottoRepo.findById(prodottoId).get();  //per ora accedo al prodotto a cui dare l'ing tramite il suo id
	...
	public Image createImage(ImageForm imageForm) throws IOException{
		...
		imgToSave.setProdotto(imageForm.getProdotto());  //setto il prodotto per questa immagine, che è lo stesso prodotto dell'imgForm che passo come paramentro
		
		//converto il multipart in un array di byte (multipart = imgForm --> byte[] = img)
		if(imageForm.getContentMultipart() != null) {
			byte[] contentSerialized = imageForm.getContentMultipart().getBytes();  //prendo il contentmultipar della form -> trasformo in byte (serializzo) -> salvo in un array di byte
	...
	//metodo che restituisce il contenuto dell'immagine a partire dal suo id (restituisce il contenuto, quindi l'array di byte, che a scermo viene mostrato come immagine
