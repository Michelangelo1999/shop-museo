...
//	}
	...
	public String delete(@PathVariable("id") Integer rifornimentoId, RedirectAttributes redAtt) {
		...
		if (result.isPresent()) {
			...
			redAtt.addFlashAttribute("successSms", "Il rifornimento del prodotto "
					+ result.get().getProdotto().getNome() + " è stato eliminato dall'assortimento");
			return "redirect:/rifornimento/add/" + result.get().getAssortimento().getId();
		} else {
			throw new ResponseStatusException(HttpStatus.NOT_FOUND,
					"Il rifornimento con id " + rifornimentoId + " non è presente nell'ordine!");
		}
	}
